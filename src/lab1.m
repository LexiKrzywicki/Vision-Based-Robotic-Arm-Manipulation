...
try
  ...
  for k = viaPts
      ...
      angles = [packet(3) packet(4) packet(5)];
      ...
      %robot.write sends a 15 float packet to the micro controller
       ...
       robot.goal_js(angles);
       robot.setpoint_js(SERVER_ID_READ, returnPacket);
      toc
      ...
      packet(1) = 1000;%one second time
      packet(2) = 0;%linear interpolation
      packet(3) = positions(1);
      packet(4) = positions(2);% Second link to 0
      packet(5) = positions(3);% Third link to 0

      % Send packet to the server and get the response      
      %robot.write sends a 15 float packet to the micro controller
      robot.write(SERV_ID, packet); 
      %robot.read reads a returned 15 float backet from the micro controller
      robot.read(SERVER_ID_READ);
